...
ex)
1. original 8bit data: 11111111

2. expand to 16bit: 1111111100101010 (00101010: garbage)

3. clear possible remaining garbage: 1111111100000000
(assuming bitmap shrink implementation in future homework)

fail if new size exceeds limit(bit count of elem_type)
*/

struct bitmap* bitmap_expand(struct bitmap* bitmap, int size){
  size_t tmpsize = bitmap->bit_cnt + size;
  //if size does not exceed bit count of elem_type
  if(tmpsize <= ELEM_BITS){
    //clear garbage
    for(int i = bitmap->bit_cnt; i < tmpsize; i++){
      bitmap_reset(bitmap, i);  //use built-in function
    }
    //append bit count
    bitmap->bit_cnt = tmpsize;

    //return resized bitmap
    return bitmap;
  }
  
  return NULL; //else fail
